...
T1[EASY]: 1. mock objects: +state, +operations ON THE FLY!
T2[HARD]: 1. mock objects + 2. patching(replacing a certain function in run time ) context [context:during the execution of a function]
...
=> 2. register: 1. register  is done successfully, 2. already registered user cannot register again
=> 3. user exists: check if a user exists or not  
=> 4. Contained list is not empty + contained List is an iterable (fetched/read)


=> [added feature] login: 1. login is done successfully, 2. user should be registered before logging in, 
5. if a user is logged in, he cannot log in again at the same time/session (unless he logs out)
6. if a user is logged in, no OTHER user can login at the same time/session
...
unit testing <=> integration testing: is the same as unit testing, plus having the actual dependencies (No mocking) 
...
class TestLogicClass(unittest.TestCase):
    ...
    def setUpClass(self):
        """
        To understand line 60: Take a look at the implementation of the LogicClass Constructor
        ...
        self.mockedDataAccessObj.return_value.read.return_value = [{'un': 'testUser', 'ps': 123}] # Fixture: Sample test list 
